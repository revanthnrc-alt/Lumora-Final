  ...
  private createPanels(): void {
    ...
    if (SITE_VARIANT === 'full') {
      const gdeltIntelPanel = new GdeltIntelPanel(() => ({
        ...
        latestClusters: this.ctx.latestClusters,
      }));
      ...
      const watchtowerPanel = new WatchtowerPanel(() => ({
        allNews: this.ctx.allNews,
        latestClusters: this.ctx.latestClusters,
        intelligenceCache: this.ctx.intelligenceCache,
      }));
      this.ctx.panels['watchtower'] = watchtowerPanel;

      const investigationPanel = new InvestigationPanel(() => ({
        allNews: this.ctx.allNews,
        latestClusters: this.ctx.latestClusters,
        intelligenceCache: this.ctx.intelligenceCache,
      }));
      this.ctx.panels['investigation'] = investigationPanel;
    }
