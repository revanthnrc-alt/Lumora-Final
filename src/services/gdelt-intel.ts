...
export async function fetchTopicIntelligence(topic: IntelTopic, timespan = '24h'): Promise<TopicIntelligence> {
  const articles = await fetchGdeltArticles(topic.query, 10, timespan);
  return {
    topic,
    articles,
    fetchedAt: new Date(),
