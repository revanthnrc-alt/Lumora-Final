...
const WS_API_URL = getEnvValue('VITE_WS_API_URL');
...
const FORCE_DESKTOP_RUNTIME = getEnvValue('VITE_DESKTOP_RUNTIME') === '1';
...
export function getApiBaseUrl(): string {
  ...
  const configuredBaseUrl = getEnvValue('VITE_TAURI_API_BASE_URL');
...
export function getRemoteApiBaseUrl(): string {
  const configuredRemoteBase = getEnvValue('VITE_TAURI_REMOTE_API_BASE_URL');
...
const APP_HOSTS = new Set([
  ...
  'localhost',
  '127.0.0.1',
  ...extractHostnames(WS_API_URL, getEnvValue('VITE_WS_RELAY_URL')),
]);
