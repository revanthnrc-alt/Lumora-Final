  ...
  if ('caches' in window) {
    void caches.keys()
      .then((keys) => Promise.all(keys
        .filter((key) => key.toLowerCase().includes('workbox') || key.toLowerCase().includes('worldmonitor'))
        .map((key) => caches.delete(key))))
      .catch(() => {});
  }
}

if (!isLocalDevHost && !('__TAURI_INTERNALS__' in window) && !('__TAURI__' in window) && 'serviceWorker' in navigator) {
