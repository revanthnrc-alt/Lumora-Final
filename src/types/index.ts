...
export type AgentRunStatus = 'idle' | 'collecting' | 'analyzing' | 'complete' | 'error';
...
export interface EvidenceItem {
  id: string;
  kind: 'headline' | 'signal' | 'region' | 'country' | 'context' | 'system';
  label: string;
  detail: string;
  source?: string;
  url?: string;
...
export interface WatchtowerAssessment {
  trigger: string;
  whyItMatters: string;
  outlook: string;
...
export interface WatchtowerAlert {
  ...
  confidence: number;
  providerMeta: AgentProviderMeta;
}
