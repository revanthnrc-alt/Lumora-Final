...
export class InsightsPanel extends Panel {
  ...
  private static readonly SENTIMENT_TIMEOUT_MS = 7000;
  private static readonly BRIEF_TIMEOUT_MS = 15000;
  private static readonly PARALLEL_TIMEOUT_MS = 8000;
  ...
  private buildFallbackBrief(clusters: ClusteredEvent[]): string {
    const top = clusters.slice(0, 3).map(cluster => cluster.primaryTitle);
    if (top.length === 0) {
      return 'Waiting for enough corroborated stories to generate a reliable brief.';
    }
    return `Top confirmed developments: ${top.join(' | ')}`;
  }
  ...
  private async updateFromServer(
    ...
    try {
      ...
      if (mlWorker.isAvailable) {
        sentiments = await this.withTimeout(
          mlWorker.classifySentiment(titles).catch(() => null),
          InsightsPanel.SENTIMENT_TIMEOUT_MS,
          null,
        );
      }
    ...
    try {
      ...
      if (!worldBrief || now - this.lastBriefUpdate > InsightsPanel.BRIEF_COOLDOWN_MS) {
        ...
        const result = await this.withTimeout(
          ...
          InsightsPanel.BRIEF_TIMEOUT_MS,
          null,
        );
      ...
      if (!worldBrief) {
        worldBrief = this.buildFallbackBrief(importantClusters);
      }
      ...
      await this.withTimeout(parallelPromise, InsightsPanel.PARALLEL_TIMEOUT_MS, undefined);
      ...
      const importantClusters = this.selectTopStories(clusters, 6);
      const fallbackBrief = this.buildFallbackBrief(importantClusters);
      this.setDataBadge('cached');
      this.renderInsights(importantClusters, null, fallbackBrief);
    }
  }
